                      ...
                      const VkRenderingInfo *pRenderingInfo)
{
   ...
   struct nv_push *p = nvk_cmd_buffer_push(
      cmd, NVK_MAX_RTS * 12 + 34 + zcull_count
   );
   ...
   if (use_zcull) {
      ...
      P_IMMD(p, NV9097, SET_ZCULL_DIR_FORMAT, {
         .zdir = ZDIR_LESS,
         .zformat = nvk_vk_format_to_zcull_format(render->depth_att.iview->vk.format),
      });
      ...
      P_IMMD(p, NV9297, SET_ZCULL_SUBREGION_REPORT_TYPE, { .enable = true, .type = TYPE_DEPTH_TEST });
