                  ...
                  uint32_t alignment, enum anv_bo_alloc_flags alloc_flags)
{
   ...
   uint64_t alloc_size = requested_size;
   ...
   if (!anv_bo_is_small_heap(alloc_flags) && (requested_size > (64 * 1024)))
    ...
    */
   if (requested_size < bo_alignment && bo_alignment <= device->info->mem_alignment)
   ...
   assert((intel_48b_address(bo->offset) & (bo_alignment - 1)) == 0);
