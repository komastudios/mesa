...
TEST_F(drirc, disable_dedicated_sparse_queue)
{
   ...
   create_device();
   enabled = is_dedicated_sparse_queue_enabled();
   destroy_device();

   /* Nothing to verify if the dedicated sparse queue isn't enabled by default. */
   if (!enabled)
      return;

   setenv("radv_disable_dedicated_sparse_queue", "true", 1);

   create_device();
   EXPECT_FALSE(is_dedicated_sparse_queue_enabled());
   destroy_device();

   unsetenv("radv_disable_dedicated_sparse_queue");
}
...
TEST_F(drirc, override_uniform_offset_alignment)
{
   ...
   setenv("radv_override_uniform_offset_alignment", "16", 1);
   ...
   unsetenv("radv_override_uniform_offset_alignment");
}
...
TEST_F(drirc, disable_depth_storage)
{
   ...
   supported = tiled_storage_features & storage_features;

   destroy_device();

   /* Nothing to verify if the depth format doesn't already support storage. */
   if (!supported)
      return;

   setenv("radv_disable_depth_storage", "true", 1);
   ...
   unsetenv("radv_disable_depth_storage");
}
...
%gl_WorkGroupSize = OpConstantComposite %v3uint %uint_1 %uint_1 %uint_1
   ...
   */
   ...
   setenv("radv_override_compute_shader_version", "1", 1);
   ...
   unsetenv("radv_override_compute_shader_version");
